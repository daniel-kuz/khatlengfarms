  ...
  return (
    <header style={{ position: 'relative', minHeight: '56vh', display: 'flex', alignItems: 'flex-end', padding: 'clamp(72px,9vw,144px) clamp(24px,5vw,96px) 64px', color: 'var(--color-bone)', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: 1320, margin: '0 auto', width: '100%' }}>
        <div className="numlabel" style={{ color: 'var(--color-gold-soft)', marginBottom: 20 }}><span>â€”</span>GOVERNANCE</div>
  ...
  const team = [
    {
      ...
      creds: 'Law firm director Â· Private equity & venture capital investment and audit committees Â· Full-time farming from May 2026',
    ...
    },
    {
      ...
      role: 'Co-Beneficiary Â· Managing Person',
      creds: 'BBA (European University of Lefke) Â· Certificate in Data Analytics (UCT) Â· Certificate in Software Development (Power Learn Project Kenya)',
    ...
    },
    {
      ...
      role: 'Co-Beneficiary Â· Financial Governance',
      creds: 'BBA (European University of Lefke) Â· Currently completing CTA Â· Serving SAICA articles at an accredited audit firm',
    ...
    },
    {
      ...
      role: 'Co-Beneficiary Â· Legal & Compliance',
      creds: 'LLB Â· Diploma in Sports Management Â· 10 years Governance, Risk & Compliance at the FSCA Â· Currently at a corporate law firm',
      bio: 'Oversees all legal, governance, and regulatory matters. Deep financial-sector compliance expertise accumulated over a decade at the Financial Sector Conduct Authority â€” a direct competitive advantage for the Trust.',
    },
  ];
        ...
        <div className="team-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(40px,6vw,80px) clamp(40px,5vw,64px)' }}>
          {team.map(({ name, role, creds, bio }) => (
            <div key={name} className="profile-card" style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 28, alignItems: 'start' }}>
              <ImagePlaceholder caption={`portrait Â· ${name.split(' ')[0].toLowerCase()} Â· b&w`} style={{ width: 180, height: 230, filter: 'grayscale(100%)' }} />
              <div>
...
function SuccessionPipeline() {
  const pipeline = [
    ...
    { name: 'Tshenolo Hlapane', current: 'Part-time â€” active vacations + farm involvement', future: 'Full-time farming role' },
    { name: 'Tshebeletso Hlapane', current: 'Part-time â€” active vacations + farm involvement', future: 'Full-time farming role' },
  ];
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--color-gold)' }}>
                {['Person', 'Current (2026)', 'Year 3â€“5 Path'].map(h => (
...
function SDGAlignment() {
  const sdgs = [
    ...
    { num: '02', name: 'Zero Hunger', desc: 'Maize, sunflower, vegetables, red meat â€” all DALRRD priority staple foods for South Africa.', bg: '#6E8146' },
    { num: '08', name: 'Decent Work', desc: '18â€“25 jobs Year 1, growing to 33â€“44 peak by Year 3. Women and youth priority.', bg: '#C9A84C', textColor: '#1A1A1A' },
    { num: '09', name: 'Industry & Innovation', desc: '4IR infrastructure, IoT monitoring, automated workflows â€” institutional-grade agri-tech.', bg: '#1A1A1A' },
